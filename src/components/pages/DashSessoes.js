...
function DashImoveis() {
  ...
  function removeProject (id) { //Metodo DELETE + msg de sucesso
    ...
    .then(() => {
      ...
      setProjectMessage('Sessão removida com sucesso!')
    })
    .catch(err => console.log(err))
  }
  ...
  return (
    <div className={styles.project_container}>
     ...
     <h1>  DashBoard Sessões    </h1>
   

     <LinkButton to="/imoveis" text="Criar novas sessões" />
     
     </div>
     <p> Aqui você pode acompanhar as sessões criadas, editar e excluir.</p>
     
      {message && < Message type="success" msg={message} />}
      {projectMessage && < Message type="success" msg={projectMessage} />}
      {!removeLoading && <Loading />}
          {removeLoading && imovel.length === 0 && (
            <p></p> //não esta funcionando
